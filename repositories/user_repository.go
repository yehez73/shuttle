...
type UserRepositoryInterface interface {
	...
	UpdateDriverDetails(tx *sqlx.Tx, details entity.DriverDetails, userUUID string) error
...
func (r *userRepository) FetchAllDrivers(offset int, limit int, sortField string, sortDirection string) ([]entity.User, entity.School, entity.Vehicle, error) {
    ...
    query := fmt.Sprintf(`
        SELECT
            ...
            COALESCE(s.school_name, 'N/A') AS school_name, COALESCE(v.driver_uuid, NULL) AS driver_uuid, COALESCE(v.vehicle_number, 'N/A') AS vehicle_number, COALESCE(v.vehicle_name, 'N/A') AS vehicle_name
        FROM users u
...
func (r *userRepository) CheckEmailExist(uuid string, email string) (bool, error) {
	var count int
	query := `SELECT COUNT(*) FROM users WHERE user_email = $1 AND deleted_at IS NULL`
...
func (r *userRepository) CheckUsernameExist(uuid string, username string) (bool, error) {
	var count int
	query := `SELECT COUNT(*) FROM users WHERE user_username = $1 AND deleted_at IS NULL`
...
func (r *userRepository) UpdateDriverDetails(tx *sqlx.Tx, details entity.DriverDetails, userUUID string) error {
	println("user_uuid: ", userUUID)
	fmt.Printf("%+v\n", details)
	query := `
		...
		WHERE user_uuid = $9`
	res, err := tx.Exec(query, details.SchoolUUID, details.VehicleUUID, details.FirstName, details.LastName, details.Gender, details.Phone, details.Address, details.LicenseNumber, userUUID)
	if err != nil {
		return err
	}
