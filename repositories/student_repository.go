...
func (repo *StudentRepository) FetchAllStudentsWithParents(offset int, limit int, sortField string, sortDirection string, schoolUUID string) ([]entity.Student, entity.ParentDetails, error) {
	...
	for rows.Next() {
		err := rows.Scan(&student.StudentUUID, &student.ParentUUID, &student.SchoolUUID, &student.StudentFirstName, &student.StudentLastName,
			&student.StudentGender, &student.StudentGrade, &student.CreatedAt, &parentDetails.UserUUID, &parentDetails.FirstName,
...
func (repo *StudentRepository) FetchSpecStudentWithParents(studentUUID uuid.UUID, schoolUUID string) (entity.Student, entity.ParentDetails, error) {
	...
	err := repo.db.QueryRowx(query, studentUUID, schoolUUID).Scan(&student.StudentUUID, &student.ParentUUID, &student.SchoolUUID, &student.StudentFirstName,
		&student.StudentLastName, &student.StudentGender, &student.StudentGrade, &student.CreatedAt, &parentDetails.UserUUID, &parentDetails.FirstName,
		&parentDetails.LastName, &parentDetails.Phone, &parentDetails.Address)
	if err != nil {
...
func (repo *StudentRepository) SaveStudent(student entity.Student) error {
	query := `INSERT INTO students (student_id, student_uuid, parent_uuid, school_uuid, student_first_name, student_last_name
		, student_gender, student_grade, created_by)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`
	res, err := repo.db.Exec(query, student.StudentID, student.StudentUUID, student.ParentUUID, student.SchoolUUID,
		student.StudentFirstName, student.StudentLastName, student.StudentGender, student.StudentGrade, student.CreatedBy)
	if err != nil {
		return err
	}
...
func (repo *StudentRepository) UpdateStudent(student entity.Student) error {
	query := `UPDATE students SET student_first_name = $1, student_last_name = $2, student_gender = $3, student_grade = $4, updated_at = NOW(), updated_by = $5
		WHERE student_uuid = $6 AND school_uuid = $7 AND deleted_at IS NULL`
	_, err := repo.db.Exec(query, student.StudentFirstName, student.StudentLastName, student.StudentGender, student.StudentGrade, student.UpdatedBy, student.StudentUUID, student.SchoolUUID)
	if err != nil {
		return err
	}
